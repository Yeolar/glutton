...
static void create_worker(void *(*func)(void *), void *arg) {
    ...
    if ((ret = pthread_create(&thread, &attr, func, arg)) != 0) {
        fprintf(stderr, "Can't create thread, %m\n");
        exit(1);
    }
}
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    if (read(fd, buf, 1) != 1)
        //erro_out( "Can't read from libevent pipe\n");
    ...
    if (NULL != item) {
        ...
        if (c == NULL) {
            //erro_out( "Can't listen for events on fd %d\n", item->sfd);
            close(item->sfd);
        }
        cqi_free(item);
    }
...
//	debug_out( "dispatch to thread[%d]\n", tid);
    ...
    if (write(thread->notify_send_fd, "", 1) != 1) {
        //erro_out("Writing to thread notify pipe %m\n");
    }
}
